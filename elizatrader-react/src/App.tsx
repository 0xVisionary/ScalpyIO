      ...
      sendMessage(scanCommand)
        .then((response) => {
          setMessages((prev) => {
            ...
            const messagesWithoutUpdates = prev.filter(
              (m) => m.type !== "update"
            );
            return [
              ...messagesWithoutUpdates,
              {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
      ...
      if (tabs[0]?.id && !tabs[0].url?.startsWith("chrome://")) {
        try {
          ...
          console.log(
            "Error sending message, attempting to inject content script:",
            error
          );
          try {
      ...
      try {
        ...
        if (success) {
          setMessages([
            {
              id: Date.now().toString(),
              text: "Hello! I'm your Scalpy, your trading assistant. How can I help you today?",
              type: "bot",
              timestamp: new Date(),
            },
          ]);
    ...
    <div className="flex flex-col min-h-0 bg-transparent text-[#0F172A] h-full w-full p-0">
      <header className="flex flex-col border-b border-gray-200 px-2">
        <div className="flex items-center py-2">
          <h1 className="text-lg font-semibold text-[#0F172A]">Scalpy</h1>
        </div>
